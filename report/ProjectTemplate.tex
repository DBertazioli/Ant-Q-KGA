...
chromosomes) and MaxG the number of generations used in the experiment\cite{venti}. The values for this parameters used in this implementation are discussed in sec.~\ref{sec:results_and_evaluation}.
\begin{table}
\centering
\begin{tabular}{@{}>{\hspace{3em}}p{.8\linewidth}@{}}
\toprule
...
\end{table}
%FIXME: add elitism and selection to spiegazione
...
\begin{itemize}
\item Keeping in mind Tab.~\ref{Tab: GA pseudocode}, mutation rate ($Tm$), is typically set to $Tm = 0.01$. This choice is explained with the aim of limiting an excessive randomization while still maintaining a good trade-off between the exploitation and exploration behaviour.
\item Population size ($Tp$) is set to $Tp = 300$ for every dataset with classic GA with the exception of \textit{pr1002}, where the dataset dimension would have not allowed such a setting considering the computational limits. In this particular case we take $Tp = 100$. For each dataset used with KGA $Tp = 100$ considering the reduced dimensionality of the sub-problems which such an algorithm is applied to.
...
\item Number of generations ($MaxIt$) is set to $MaxIt = 1000$ in classic GA except for \textit{pr1002} where $MaxIt = 5000$ attempting to achieve a decent performance on such a large dataset. Instead, in KGA a larger value of $MaxIt = 5000$ is used for each dataset still considering the dimensional reduction associated with the clustering.
\item Number of clusters
%FIXME: aggiungere n_cluster, elitismo in pseudocode
%FIXME: check with max 
\end{itemize}

Tab.~\ref{tab:ga_results} presents the results of such a testing procedure for the GA algorithm, whereas Tab.~\ref{tab:kga_results} provides the same information for the KGA approach.
...
\caption{\label{tab:ga_results}The resulting Ant-Q performances on the test datasets.}
\end{table}
...
\caption{\label{tab:kga_results}The resulting Ant-Q performances on the test datasets.}
\end{table}
...
For as regards the evolutionary approach, the following conclusions are drawn:
\begin{itemize}
\item FIXME: kek
\end{itemize}
