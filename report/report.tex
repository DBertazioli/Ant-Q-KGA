...
\vspace*{5cm}
\begin{abstract}
In order to face a non-linear optimization problem, especially a combinatorial one, and the state-of-the-art in metaheuristics and hybridization, the travelling salesman problem (TSP), in particular in its symmetric version, was chosen, together with two specific algorithms: Ant Colony and Genetic Algorithm.
The main goal of this work consists of applying the two algorithms on TSP, referring to the abundant implementations in literature concerning these specific algorithms. 
After that, our objective is to attempt an improvement through an hybrid version of each algorithm, in particular combining Ant Colony together with elements of Reinforcement Learning i.e. Q-Learning algorithm, while Genetic Algorithm are integrated along with techniques of dimensionality reduction, such as clustering.
A series of tests and comparisons are performed, not only between each algorithm and its hybrid version, where the hybridization is proven to provide a relevant performance boost, but also among the two different sets of algorithms. In such a comparison, interesting results are found, suggesting the first Ant-type algorithms to be better suitable in cases of performant hardware or low-dimensional datasets, whereas the second evolutionary approach combined with clustering to be better performant on large datasets and especially in those situations where the objective is an exploratory analysis of the problem, requiring a responsible approach in a reasonable (cpu-)time.
%FIXME: giusto un tentativo, non so se vi possa piacere l'impostazione.
...
In particular, ACO algorithm  is based on  emulating the  behaviour  of  some  ant species.  
...
Cooperation  among  the  ants  has inspired researchers to apply a similar cooperation based algorithm to those problems whose solutions can be formulated as a shortest-path  problem between an origin and a destination.
...
A ``fitness" function is used to evaluate each individual to decide whether it will contribute to the next generation. 
...
\end{itemize}
\item \textbf{Update the pheromone matrix}:  the   pheromone   trails   are   adjusted   considering the latest iteration of the colony search process.  
...
In our implementation, and following the literature, $\rho$ is taken equal to $0.5$. Such a value should represent a compromise between exploring and exploiting the panorama of possible solutions;
...
\label{eq:localupd}
\end{equation}
%FIXME: @dario questa equazione non è la medesima di Eq.11?
...
\end{thebibliography}
%FIXME: add aknoledgments and APPENDIX WITH LINKS VARI
\begin{appendix}
\appendix
