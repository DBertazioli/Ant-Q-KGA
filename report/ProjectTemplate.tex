...
In fact GA for small and KGA for larger datasets, represents a good solution for initial approach to a combinatorial problem, in order to save computational time in order to achieve pretty good approximate solutions, maybe with the aim to find a good starting point for another algorithm.
Instead, if the aim is to try to find the global optimal solution, and especially in condition to have powerful hardware and enough time, then Ant family algorithms are more suited.
\end{itemize}
\begin{figure}
