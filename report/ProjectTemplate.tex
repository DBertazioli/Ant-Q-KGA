...
\item $N_i^k$ the neighbourhood region of the k-th ant.
...
The $q_0$ value is analogous to the $\epsilon_0$ parameter in the well known $\epsilon$-greedy reinforment learning policy. Following this analogy, the $q$ variable is simply taken as a random number. When the condition $q<q_0$ is not met, a decision process correspondent to Eq.~\ref{eq:prob-aco} applies. In this work, following the main reference \cite{undici}, this parameter is taken as $q_0=0.9$ (minor tuning attemps were made, not leading to an apparently better setting).
